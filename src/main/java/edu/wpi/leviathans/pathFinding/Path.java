...
public class Path implements Iterable<Node> {
    ...
    public void add(Node node) {
        Edge lastEdge = null;
        if(pathNodes.size() > 0) lastEdge = pathNodes.get(pathNodes.size() - 1).getEdge(node);

        if(pathNodes.size() == 0 || lastEdge != null) {
            pathNodes.add(node);
            length += lastEdge.length;
        } else {
            throw new IllegalArgumentException("Last node does not have an Edge pointing to the given node");
        }
    }
    ...
    public void addFirst(Node node) {
        ...
        if(pathNodes.size() == 0 || firstEdge != null) {
            ...
            throw new IllegalArgumentException("Given node does not have an edge pointing to the previous first node");
        }
    }
    ...
    /**
     * Converts an ordered list of nodes to a path. Ensure that there is an actual path of edges between the nodes.
